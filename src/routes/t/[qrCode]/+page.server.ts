...
export const load: PageServerLoad = async ({ params, url }) => {
	const apiUrl = `${API_BASE_URL}/api/v1/public/tickets/${params.qrCode}`;
	let res: Response;
	try {
		...
		console.error('[t] fetch threw', { url: apiUrl, message, cause });
		throw error(502, `Upstream fetch failed: ${message}${cause ? ` (${cause})` : ''}`);
	}
	if (!res.ok) {
		const body = await res.text().catch(() => '');
		console.error('[t] upstream non-ok', { url: apiUrl, status: res.status, body });
